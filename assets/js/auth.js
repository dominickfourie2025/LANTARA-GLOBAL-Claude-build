    ...
    signupForm.addEventListener('submit', async function (e) {
      ...
      try {
        ...
        var signUpResult = await sb.auth.signUp({
          ...
          password: password,
          options: {
            // See employer-auth.js for why this must be explicit —
            // otherwise Supabase falls back to the bare page origin
            // and drops the /LANTARA-GLOBAL-Claude-build/ repo path,
            // 404-ing confirmed users. Must also be added to
            // Supabase's Redirect URLs allow-list.
            emailRedirectTo: window.location.origin + window.location.pathname.replace(/[^/]+$/, '') + 'teacher-sign-in.html',
            data: {
              role: 'teacher',
